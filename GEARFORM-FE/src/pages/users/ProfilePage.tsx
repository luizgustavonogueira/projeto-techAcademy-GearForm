...
function ClipCard({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={{
      background: C.card,
...
function NeonBtn({ children, onClick, small }: any) {
...
function NeonInput({ label, value, onChange, type = "text", error, hint, disabled }: any) {
...
export default function ProfilePage({ user, onUpdateUser }: ProfilePageProps) {
  const [nome,    setNome]    = useState(user.nome);       // ← usa user.nome (alinhado com o tipo)
  ...
  return (
    <div style={{ padding:28, animation:"gf-fadeup 0.35s ease", maxWidth:600 }}>
      ...
      <ClipCard style={{ padding:20, marginBottom:24, display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <NeonInput
          label="Senha Atual" type="password" value={oldPwd}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setOldPwd(e.target.value)}
          error={errors.oldPwd}
        />
        <NeonInput
          label="Nova Senha" type="password" value={newPwd}
        ...
        <NeonInput
          label="Confirmar Nova Senha" type="password" value={confPwd}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setConfPwd(e.target.value)}
          error={errors.confPwd}
        />
      </ClipCard>
