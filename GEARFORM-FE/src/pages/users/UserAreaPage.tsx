...
const makeQuestions = (titulo: string, modIdx: number) => [
...
function Tag({ children, color=C.accent }: any) {
  return <span style={{ display:"inline-block", padding:"2px 8px", fontFamily:"'Space Mono',monospace",
    fontSize:9, letterSpacing:"0.08em", color, border:`1px solid ${color}30`,
    background:`${color}0f`, textTransform:"uppercase", lineHeight:1.9, whiteSpace:"nowrap" }}>{children}</span>;
}

function ClipCard({ children, style={}, onClick, glow=false }: any) {
...
function NeonBtn({ children, onClick, disabled, small, variant="primary", type="button" }: any) {
  const [hov, setHov] = useState(false);
...
function NeonInput({ label, value, onChange, type="text", error, hint, disabled, maxLength, placeholder }: any) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
      {label && <label style={{ fontFamily:"'Space Mono',monospace", fontSize:9, letterSpacing:"0.2em",
      ...
      <div style={{ position:"relative" }}>
        ...
        {isPwd&&<button type="button" onClick={()=>setShowPwd(v=>!v)} style={{ position:"absolute",right:12,
...
function ProgressBar({ value, color=C.accent }: any) {
  return (
    <div style={{ height:3, background:"rgba(255,255,255,0.06)", borderRadius:2, overflow:"hidden" }}>
      <div style={{ height:"100%", width:`${value}%`, background:color,
        transition:"width 0.8s ease", boxShadow:`0 0 8px ${color}80` }}/>
    </div>
  );
}
          ...
          <>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:9,marginBottom:18 }}>
              {q.opcoes.map((op:string,i:number)=>{
                ...
                if(ans){ if(i===q.correta){bg=`${C.accent}15`;bord=C.accent;col=C.accent;}
                  else if(i===sel){bg=`${C.pink}15`;bord=C.pink;col=C.pink;} }
                else if(i===sel){bg=`${C.violet}15`;bord=C.violet;}
                return <button key={i} onClick={()=>!ans&&setSel(i)} style={{ background:bg,border:`1px solid ${bord}`,
                  color:col,padding:"10px 14px",fontSize:13,fontFamily:"'Outfit',sans-serif",textAlign:"left",
            ...
            {ans&&<div style={{ padding:"10px 14px",background:`${C.violet}0f`,
...
/* ─── TRAIL VIEW ─────────────────────────────────────────────────────────── */
function TrailView({ curso, mat, onUpdate, onBack }: any) {
  ...
  return (
    <div style={{ padding:28, animation:"gf-fadeup 0.35s ease", maxWidth:720 }}>
      {quiz!==null&&<QuizModal curso={curso} modIdx={quiz} onClose={()=>setQuiz(null)}
        onComplete={()=>complete(quiz)}/>}
      ...
      <ClipCard style={{ padding:16,marginBottom:22 }}>
        ...
        {mat.progresso===100&&<p style={{ margin:"10px 0 0",fontSize:11,color:C.accent,
...
/* ─── EXPLORE VIEW ───────────────────────────────────────────────────────── */
function ExploreView({ mats, setMats, goTrail }: any) {
  ...
  const enroll = (id:number) => {
    if(mats.find((m:any)=>m.cursoId===id)) return;
    setMats((p:any)=>[...p,{cursoId:id,progresso:0,status:"ativo",modulos_feitos:[]}]);
  ...
  return (
    <div style={{ padding:28,animation:"gf-fadeup 0.35s ease" }}>
      ...
      <div style={{ display:"flex",gap:10,marginBottom:18,flexWrap:"wrap" }}>
        ...
        {[{v:cat,set:setCat,opts:CATS},{v:nivel,set:setNivel,opts:NIVEIS}].map((s,i)=>(
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14 }}>
        {filtered.map(curso=>{
          const mat = mats.find((m:any)=>m.cursoId===curso.id);
          const enrolled = !!mat;
          return (
...
/* ─── MY COURSES VIEW ────────────────────────────────────────────────────── */
function MyCoursesView({ mats, setMats, openTrailId, setOpenTrailId }: any) {
  ...
  const updMat = (nm:any) => setMats((p:any[])=>p.map(m=>m.cursoId===nm.cursoId?nm:m));

  if(trailCurso){
    const mat=mats.find((m:any)=>m.cursoId===trailCurso.id)!;
        ...
        <>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:12,marginBottom:26 }}>
            {[
              ...
              {label:"Em andamento", val:mats.filter((m:any)=>m.progresso<100&&m.progresso>0).length, color:C.violet},
              {label:"Concluídos", val:mats.filter((m:any)=>m.progresso===100).length, color:"#f59e0b"},
            ].map(s=>(
...
/* ─── PROFILE VIEW ───────────────────────────────────────────────────────── */
function ProfileView({ user, onUpdateUser }: any) {
  ...
  const validate = () => {
    ...
    else if(nome.trim().length<3) e.nome="Mínimo 3 caracteres";
    if(newPwd){
  ...
  return (
    <div style={{ padding:28,animation:"gf-fadeup 0.35s ease",maxWidth:560 }}>
      ...
      <div style={{ marginBottom:26 }}>
        ...
        <h1 style={{ margin:0,fontFamily:"'Bebas Neue',cursive",fontSize:34,
          color:C.text,letterSpacing:"0.04em",lineHeight:1 }}>MEU PERFIL</h1>
      </div>
      ...
      <ClipCard style={{ padding:20,marginBottom:16 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:20 }}>
          <div onClick={()=>fileRef.current?.click()} style={{ width:74,height:74,borderRadius:"50%",
            flexShrink:0,cursor:"pointer",border:`2px solid ${C.accent}40`,overflow:"hidden",
            position:"relative",background:`linear-gradient(135deg,${C.violet},${C.accent})`,
...
function Sidebar({ view, setView, user, collapsed, setCollapsed, onLogout, matsCount }: any) {
  return (
    <aside style={{ width:collapsed?56:220,flexShrink:0,background:C.surface,
      ...
      <div style={{ padding:collapsed?"14px 0":"18px 18px 14px",display:"flex",alignItems:"center",
        ...
        {!collapsed&&<span style={{ fontFamily:"'Bebas Neue',cursive",fontSize:20,letterSpacing:"0.08em",
      ...
      <nav style={{ flex:1,padding:"10px 0" }}>
        {NAV.map(n=>{
          ...
          return (
            <button key={n.id} onClick={()=>setView(n.id)} style={{ display:"flex",alignItems:"center",
              gap:12,width:"100%",padding:collapsed?"12px 0":"10px 18px",
        ...
        {!collapsed&&(
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            ...
            <div style={{ minWidth:0 }}>
              <p style={{ margin:0,fontSize:12,fontWeight:600,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{user.nome}</p>
              <p style={{ margin:0,fontSize:10,color:C.muted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{user.email}</p>
            </div>
          </div>
        )}
        <button onClick={onLogout} style={{ display:"flex",alignItems:"center",
          gap:8,background:"none",border:"none",cursor:"pointer",width:"100%",
          padding:collapsed?"8px 0":"4px 0",justifyContent:collapsed?"center":"flex-start",
          fontFamily:"'Space Mono',monospace",fontSize:10,letterSpacing:"0.1em",
          color:"rgba(255,255,255,0.25)",textTransform:"uppercase",transition:"color 0.18s" }}
      ...
      {/* Toggle */}
      <button onClick={()=>setCollapsed((v:boolean)=>!v)} style={{ position:"absolute",
        top:24,right:-10,width:20,height:20,borderRadius:"50%",
...
/* ─── TOPBAR ─────────────────────────────────────────────────────────────── */
function Topbar({ view, user, onMenuClick, sidebarOpen }: any) {
  ...
  return (
    <header style={{ height:56,background:C.surface,borderBottom:`1px solid ${C.border}`,
      ...
      <div style={{ display:"flex",alignItems:"center",gap:14 }}>
        <button onClick={onMenuClick} style={{ background:"none",border:`1px solid ${C.border}`,
          cursor:"pointer",color:C.muted,padding:"5px 8px",display:"flex",
...
export default function UserAreaPage() {
  ...
  const handleUpdateUser = (u:any) => updateUser(u);

  const goTrail = (id: number) => {
    setOpenTrailId(id);
    setView("myCourses");
  };
      ...
      <div style={{ display:"flex",height:"100vh",background:C.bg,
        ...
        <div style={{ flex:1,display:"flex",flexDirection:"column",overflow:"hidden" }}>
          <Topbar view={view} user={user}
            onMenuClick={()=>setCollapsed(v=>!v)} sidebarOpen={!collapsed}/>
          <main style={{ flex:1,overflowY:"auto",overflowX:"hidden" }}>
            {view==="explore"&&<ExploreView mats={mats} setMats={setMats} goTrail={goTrail}/>}
            {view==="myCourses"&&<MyCoursesView mats={mats} setMats={setMats}
              openTrailId={openTrailId} setOpenTrailId={setOpenTrailId}/>}
